...
int CVIFUNC ComRd(int portNo, char buf[], size_t maxCnt);
int CVIFUNC ComRdByte(int portNo);
int CVIFUNC ComRdTerm(int portNo, char buf[], size_t   maxCnt, int termCh);
int CVIFUNC ComToFile(int portNo, int handle, int   maxCnt, int termCh);
int CVIFUNC ComWrt(int portNo, const char *buf, size_t maxCnt);
...
int CVIFUNC GetSystemComHandle(int portNo, intptr_t *handle);
...
typedef enum {
    ...
    kRS_NullPointerPassed        =  -8,
    kRS_OutOfMemory              =  -11,
    kRS_OutOfSystemResources     =  -12,

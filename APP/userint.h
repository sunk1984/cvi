...
/*----------------------------------------------------------------------------*/
/*    Copyright (c) National Instruments 1987-2011.  All Rights Reserved.     */
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Title:       userint.h                                                     */
...
#ifndef CVI_FILESYSTEM_VALS_DEFINED
#define CVI_FILESYSTEM_VALS_DEFINED
...
#endif /* CVI_FILESYSTEM_VALS_DEFINED */
...
#if defined(_NI_mswin64_)
#define VAL_SIZE_T                      VAL_UNSIGNED_64BIT_INTEGER
#define VAL_SSIZE_T                     VAL_64BIT_INTEGER
#define VAL_PTRDIFF_T                   VAL_64BIT_INTEGER
#define VAL_UINTPTR_T                   VAL_UNSIGNED_64BIT_INTEGER
#define VAL_INTPTR_T                    VAL_64BIT_INTEGER
#else
#define VAL_SIZE_T                      VAL_UNSIGNED_INTEGER
#define VAL_SSIZE_T                     VAL_INTEGER
#define VAL_PTRDIFF_T                   VAL_INTEGER
#define VAL_UINTPTR_T                   VAL_UNSIGNED_INTEGER
#define VAL_INTPTR_T                    VAL_INTEGER
#endif

    /*** special position values: ***/
#define VAL_AUTO_CENTER         (-2147483647L - 1) /* for ATTR_LABEL_TOP, ATTR_, ... */
                                                    /* SetPanelPos, NewPanel, ... */
#define VAL_RIGHT_ANCHOR        -2147483633L /* for ATTR_LABEL_LEFT, ATTR_RING_ARROW_LOCATION, AddCtrlToSplitter... */
#define VAL_LEFT_ANCHOR         -2147483634L /* for ATTR_LABEL_LEFT, ATTR_RING_ARROW_LOCATION, AddCtrlToSplitter... */
#define VAL_BOTTOM_ANCHOR       -2147483635L /* for ATTR_LEGEND_TOP, AddCtrlToSplitter... */
#define VAL_TOP_ANCHOR          -2147483636L /* for ATTR_LEGEND_TOP, AddCtrlToSplitter... */
#define VAL_AUTO_RANGE          -2147483637L /* for ATTR_TOP_RANGE, ... */
#define VAL_NO_RANGE_LIMIT      -2147483638L /* for ATTR_TOP_RANGE, ... */
#define VAL_KEEP_SAME_POSITION  -2147483647L /* for DuplicateCtrl, DuplicatePanel */
#define VAL_INVALID_COORDINATE  (-2147483647L - 1) /* for EVENT_CTRL_MENU */
...
#define VAL_CENTER_CENTER_JUSTIFIED     0xA0L
    ...
    /*** justification values (for ATTR_LABEL_JUSTIFY, ATTR_TEXT_JUSTIFY and ATTR_SBCELL_JUSTIFY): ***/
...
#define VAL_BASE_ZERO_VERTICAL_BAR      8L
...
#define VAL_CELL_CHECK_BOX        6
#define VAL_CELL_COLOR_PICKER     7
#define VAL_CELL_HORIZONTAL_BAR   8
...
#define VAL_CELL_MENU_ARROW_AREA             17
#define VAL_CELL_BUTTON_AREA                 18
#define VAL_CELL_CHECK_BOX_AREA              19
#define VAL_CELL_COLOR_PICKER_AREA           20
...
/*** values for font character sets ***/
#define VAL_NATIVE_CHARSET          -1
#define VAL_ANSI_CHARSET            0
#define VAL_SYMBOL_CHARSET          2
#define VAL_SHIFTJIS_CHARSET        128
#define VAL_HANGUL_CHARSET          129
#define VAL_GB2312_CHARSET          134
#define VAL_CHINESEBIG5_CHARSET     136
#define VAL_HEBREW_CHARSET          177
#define VAL_ARABIC_CHARSET          178
#define VAL_GREEK_CHARSET           161
#define VAL_TURKISH_CHARSET         162
#define VAL_VIETNAMESE_CHARSET      163
#define VAL_THAI_CHARSET            222
#define VAL_EASTEUROPE_CHARSET      238
#define VAL_RUSSIAN_CHARSET         204
#define VAL_MAC_CHARSET             77
#define VAL_BALTIC_CHARSET          186
#define VAL_OEM_CHARSET             255
...
#define MOUSE_WHEEL_SCROLL_LEFT     4
#define MOUSE_WHEEL_SCROLL_RIGHT    5
...
typedef enum
{
    ...
    UIESubFileReadingOrParsingError                             = -134,
    ...
    UIEAttrFixedPlotAreaNotEnabled                              = -173,
    UIEStaleData                                                = -174,
    UIEDataBindingNotSupportedIn64Bit                           = -175,
    UIEActiveXWrongConcurrencyModelForThread                    = -176,
    UIECtrlNotOnPanelAssocWithCtrlArray                         = -177,
    UIEErrorLimit                                               = -178
} UIError;
...
int CVIFUNC SetPanelCharacterSet(int panel, int charSet);
...
int CVIFUNC SetTextCtrlHeight(int panel, int control, int width);
...
#pragma soft_reference(GetPanelAttribute);
#endif
...
#elif defined(_NI_mswin64_)
 typedef unsigned long long WinMsgWParam;
 typedef unsigned long long WinMsgLParam;
#else
...
int CVIFUNC PostDeferredCallToThread(DeferredCallbackPtr funcPtr, void *callbackData,
                                     unsigned targetThreadId);  /* may be called asynchronously */
...
intptr_t CVIFUNC GetCVIWindowHandle(void);
intptr_t CVIFUNC GetCVIWindowHandleForCurrThread(void);
#endif
#if defined(_NI_mswin16_)
...
int CVIFUNC GetTimerTickData(double *eventTime, double *timeSinceLastCallback);
...
#if defined(_CVI_DEBUG_) && !defined(_LINK_CVI_LVRT_)
#pragma soft_reference(FormatDateTimeString);
#endif
...
int CVIFUNC PlotPolygon(int panel, int control,  void *xArray,  void *yArray,
                        size_t numPoints, int xDType, int yDType, int frameColor,
                        int fillColor);
...
int CVIFUNC PlotArc(int panel, int control, double x1, double y1, double x2,
                    ...
                    int fillColor);
int CVIFUNC PlotIntensity(int panel, int control,  void *zArray, size_t numXPoints,
                          size_t numYPoints, int zDType, ColorMapEntry colors[], int hiColor,
                          size_t numColors, int interpColors, int interpPixels);
int CVIFUNC PlotScaledIntensity(int panel, int control,  void *zArray,
                                size_t numXPoints, size_t numYPoints, int zDType, double yGain,
                                double yOffset, double xGain, double xOffset,
                                ColorMapEntry colors[], int hiColor, size_t numColors,
...
int CVIFUNC PlotStripChart(int panel, int control, void *yArray,
                           size_t numPoints, int start, int skip, int yDType);
int CVIFUNC PlotStripChartPoint(int panel, int control, double y);
int CVIFUNC PlotPackedDigitalLines (int panel, int control, void *data, size_t numElements,
                                    int dataType, void *mask);
int CVIFUNC PlotDigitalLines (int panel, int control, void *data, size_t numPoints,
                              int dataType, unsigned char numLines);
int CVIFUNC PlotPackedDigitalLinesMultiBus (int panel, int control, void *data, size_t numElements,
                                            int numBuses, int dataType, int interleaved, void *maskArray);
int CVIFUNC PlotDigitalLinesMultiBus (int panel, int control, void *data, size_t numPoints, int numBuses,
...
int CVIFUNC RefreshGraph(int panel, int control);
    ...
    /*** for graph and strip chart legends: ***/
int CVIFUNC ClearLegend(int panel, int control);
int CVIFUNC GetNumLegendItems(int panel, int control, int *numItems);
int CVIFUNC GetLegendItemFromPoint(int panel, int control, Point coordinate, int *item);
...
int CVIFUNC GetAxisTimeFormat(int panel, int control, int axis, int format, char *outputBuffer, size_t bufferSize);
...
int CVIFUNC_C SetAnnotationAttribute(int panel, int control, int index, int attribute, ...);
...
int CVIFUNC PromptPopup(const char *popupTitle, const char *messageString, char *responseBuffer, size_t maxResponseLength);
int CVIFUNC GenericMessagePopup(const char *popupTitle, const char *messageString, const char *btn1Label, const char *btn2Label,
                                const char *btn3Label, char *responseBuffer, size_t maxResponseLength,
                                int putBtnsOnSide, int activeControl, int defaultBtn, int cancelBtn);
int CVIFUNC YGraphPopup(const char *popupTitle, void *yArray, size_t numPoints, int yDType);
int CVIFUNC XGraphPopup(const char *popupTitle, void *xArray, size_t numPoints, int xDType);
int CVIFUNC XYGraphPopup(const char *popupTitle, void *xArray, void *yArray, size_t numPoints, int xDType,
                        int yDType);
int CVIFUNC WaveformGraphPopup(const char *popupTitle, void *wFmArray, size_t numPoints, int wFmDType,
...
int CVIFUNC FontSelectPopupEx(const char *popupTitle, const char *sampleString, int monoSpacedOnly, char *typeFace,int *bold,
                            int *underline, int *strikeOut, int *italic, int *justification, int *textColor, int *angle, 
                            int *charSet, int *fontSize, int minFontSize, int maxFontSize, int allowDefaultBtn, int allowMetaFonts);
int CVIFUNC SetFontPopupDefaultsEx(const char *typeFace, int bold, int underline, int strikeOut, int italic,
                                 int justification, int textColor, int angle, int charSet, int fontSize);
...
int CVIFUNC CreateMetaFontWithCharacterSet(const char *name, const char *fontName, int size, int bold,
                             int italic, int underline, int strikeout, int angle, int characterSet);
...
int CVIFUNC SetOwnerForNextCreatedWindow(intptr_t ownerHwnd);
...
int     CVIFUNC CanvasDrawPoly(int panel, int control, size_t numPts, const Point points[], int wrap, int drawMode);
...
int     CVIFUNC GetNumImagesInFile (const char fileName[], int *numImages);
int     CVIFUNC GetBitmapFromFile(const char fileName[], int *bitmap);
int     CVIFUNC GetBitmapFromFileEx(const char fileName[], int index, int *bitmap);
...
int     CVIFUNC SetRowHeightToTallestCellContents (int panel, int control, int rowIndex);
...
int     CVIFUNC GetNumTreeCellRingItems (int panel, int control, int item, int column, int *numRingItems);
int     CVIFUNC InsertTreeCellRingItem (int panel, int control, int item, int col, 
                                        int ringIndex, const char *value);
int     CVIFUNC DeleteTreeCellRingItems (int panel, int control, int item, int col, int ringIndex, 
                                         int numRingItemsToDelete);
int     CVIFUNC GetTreeCellRingIndexFromValue (int panel, int control, int item, int col, 
                                               int begRingIndex, int *ringIndex, const char *value);
int     CVIFUNC GetTreeCellRingValueFromIndex (int panel, int control, int item, int col,
                                               int ringIndex, char *value);
int     CVIFUNC GetTreeCellRingValueLengthFromIndex (int panel, int control, int item, int col, 
                                                     int ringIndex, int *length);
int     CVIFUNC SetTreeCellRingValueFromIndex (int panel, int control, int item, int col, 
                                             int ringIndex);
int     CVIFUNC GetActiveTreeCell (int panel, int control, int *item, int *col);
...
int     CVIFUNC DiscardNumericColorRamp(int panel, int control);
    ...
    /**************************************/
int     CVIFUNC NewCtrlArray(int panel);
int     CVIFUNC DiscardCtrlArray(int ctrlArray);
int     CVIFUNC InsertCtrlArrayItem(int ctrlArray, int panel, int ctrlToAdd, int index);
int     CVIFUNC DeleteCtrlArrayItem(int ctrlArray, int index, int count);
int     CVIFUNC ClearCtrlArray(int ctrlArray);
int     CVIFUNC GetCtrlArrayItem(int ctrlArray, int index);
int     CVIFUNC GetNumCtrlArrayItems(int ctrlArray, int *count);
int     CVIFUNC GetCtrlArrayIndex(int ctrlArray, int panel, int ctrl, int *index);
int     CVIFUNC GetCtrlArrayPanel(int ctrlArray);
int     CVIFUNC GetCtrlArrayFromResourceID(int panel, int ctrlArrayResourceID);
int     CVIFUNC MoveCtrlArray(int ctrlArray, int xDelta, int yDelta);
int     CVIFUNC GetCtrlArrayBoundingRect(int ctrlArray, int *top, int *left, int *height, int *width);
int     CVIFUNC_C SetCtrlArrayAttribute(int ctrlArray, int attribute, ...);
int     CVIFUNC_C SetCtrlArrayVal(int ctrlArray, ...);
